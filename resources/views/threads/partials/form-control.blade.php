<div>
    <label for="title" class="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-300">Thread Title</label>
    <input type="text" id="title" name="title"
        class="border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
        placeholder="Post something..." value="{{ old('title', $thread->title ?? '') }}" required>
    @error('title')
...
<div>
    <label for="type" class="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-400">Thread Type</label>
    <select id="type" name="type"
        ...
        <option disabled selected>-- Select Thread Type --</option>
        @foreach ($types as $type)
            <option value="{{ $type->id }}"
                {{ $type->id == old('type', $thread->thread_category_id ?? '') ? 'selected' : '' }}>
                {{ $type->name }}</option>
        @endforeach
    </select>
    @error('type')
...
<div>
    ...
    <textarea id="description" rows="8" name="description"
        ...
        placeholder="Your message...">{{ old('description', $thread->description ?? '') }}</textarea>
    @error('description')
        <p class="mt-2 text-sm text-red-600 dark:text-red-500"><span class="font-medium">Oops!</span>{{ $message }}
        </p>
    @enderror
